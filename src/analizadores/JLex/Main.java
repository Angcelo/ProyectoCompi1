/**************************************************************
  ...
  Version 1.2.5, 7/25/99-5/16/00, [C. Scott Ananian]
   ...
   Fixed "newline in quoted string" error when /* or 
...
class CSimplifyNfa
{
  ...
  void simplify(CSpec m_spec) {
	...
	CSet ncset = new CSet();
	ncset.map(nfa.m_set, ccls); 
	nfa.m_set = ncset;
      } else { 
	nfa.m_edge = ccls[nfa.m_edge]; 
      }
    }
  ...
  private void computeClasses(CSpec m_spec) {
    ...
    this.ccls = new int[original_charset_size]; 
...
class CAlloc
{
  ...
  static CNfaPair newNLPair(CSpec spec) {
    ...
    pair.m_end=newCNfa(spec); 
    pair.m_start=newCNfa(spec); 
